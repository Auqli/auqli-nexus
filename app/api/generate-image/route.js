...
export async function POST(request) {
  try {
    ...
    // Get the prompt enhancer based on type
    let promptEnhancer = ImageGenerationConfig.promptEnhancers.general
    if (promptEnhancerType === "product") {
      promptEnhancer = ImageGenerationConfig.promptEnhancers.product
    } else if (promptEnhancerType === "social") {
      promptEnhancer = ImageGenerationConfig.promptEnhancers.social
    }
   ...
   ${promptEnhancer} ${toneAddition} ${creativityLevel} ${modelConfig.promptEnhancement}`.trim()
    ...
    const enhancedNegativePrompt = modelConfig.negativePrompt
