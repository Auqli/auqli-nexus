...
export function UnifiedPromptAssistant({ initialPrompt, onPromptGenerated, useCase }) {
  ...
  const [promptQuality, setPromptQuality] = useState("minimal")
  ...
  const generateExpandedPrompt = () => {
    ...
    if (productCategory && productCategory !== "none") {
      ...
      if (categoryDetails[productCategory]) {
        expanded += `. ${categoryDetails[productCategory]}`
      }
    }
